...
<div class="container">
	...
	<c:forEach var="dto" items="${ detail_list}">
		<form
			action="udateuploadMultiFileOk?hostenter_name=${dto.hostenter_name }"
			method="post" enctype="multipart/form-data"
			onsubmit="return confirmFileExtension(imgFile.value);">
			<input type="hidden" value="${dto.hostenter_number}"
				name="hostenter_number" /> <input type="hidden"
				value="${dto.hostenter_name }" name="none_hostenter_name" />
			...
			<div>
				...
				<h2 id="space_host_info">
					호스트님께서 등록하신 공간에 대해 수정하고 확인하는 페이지 입니다.<br> 각 입력란에 맞는 규격으로 아래의
					공간 상세정보를 수정하시기 바랍니다
				</h2>
			</div>
			...
			<div class="space_host_img">
				...
				<label for="imgFile2">파일선택</label> <input name="filename2"
					type="file" id="imgFile2" accept="image/*"
					onchange="readURL(this);" />
				<div>
					<label for="imgFile4" id="delete_img2"
						onclick="fileReset2(this.form)">지우기</label>
				</div>
			</div>
			...
			<div class="space_host_name">
				...
				<input type="text" value="${dto.hostenter_name }" id="host_input"
					name="host_name">
			...
			<div class="space_host_select">
				...
				<div class="div_select">
					<div class="div_ch">
						<input type="checkbox" style="zoom: 10.5;" name="room" value="1"
							<c:if test="${dto.hostenter_contents_number eq '1' }"> checked</c:if>
							id="ch" onclick='checkOnlyOne(this)'>
					</div>
					<label id="ch1">파티룸</label>
					<div class="div_ch">
						<input type="checkbox" style="zoom: 10.5;" name="room" value="2"
							<c:if test="${dto.hostenter_contents_number eq '2' }"> checked</c:if>
							id="ch" onclick='checkOnlyOne(this)'>
					</div>
					<label id="ch1">엑티브룸</label>
					<div class="div_ch">
						<input type="checkbox" style="zoom: 10.5;" name="room" value="3"
							<c:if test="${dto.hostenter_contents_number eq '3' }"> checked</c:if>
							id="ch" onclick='checkOnlyOne(this)'>
					</div>
					<label id="ch1">슈팅룸</label>
					<div class="div_ch">
						<input type="checkbox" style="zoom: 10.5;" name="room" value="4"
							<c:if test="${dto.hostenter_contents_number eq '4' }"> checked</c:if>
							id="ch" onclick='checkOnlyOne(this)'>
					</div>
			...
			<div class="space_host_onerow">
				...
				<input type="text" value="${dto.hostenter_onerow }" id="host_input"
					name="host_onerow">
			</div>
...
<<<<<<< HEAD
			<div class="space_host_img">
				...
				<label for="imgFile">파일선택</label> <input name="filename" type="file"
					id="imgFile" accept="image/*" onchange="setDetailImage(event);"
					multiple="multiple" />
...
>>>>>>> parent of 1b669b3 (cc)
			...
			<div class="space_host_price">
				<h3 id="space_host_h3">가격</h3>
				<input type="text" value="${dto.hostenter_price }" id="host_input"
					name="host_price">
			</div>
			...
			<div class="space_host_bnum">
				<h3 id="space_host_h3">사업자등록번호</h3>
				<input type="text" value="${dto.hostenter_bnumber}" id="host_input"
					name="host_bnumber">
			</div>
		...
		<div class="space_finish" style="margin-top: 50px">
			<div>
				<button class="delebtn" 
				onclick="location.href='/delete_space?hostenter_number=${dto.hostenter_number}&hostenter_img_name=${dto.hostenter_name }'">
				공간 삭제
				</button>
			</div>
		</div>
